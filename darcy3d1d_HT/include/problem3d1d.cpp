...
try {
	...
	for (cv << nn; cv != bgeot::size_type(-1); cv << nn) {
		...
		if (meshv.convex_to_point(i0).size()==1){ /* inflow extremum */
			...
			size_type branch = 0; 
            //!!!!!Here we are looking for the first branch which contains i0 and we add it then as an inflow branch for the Junction i0. But why the first branch that I find should be of inflow???
			bool contained = false;
			while (!contained && branch<nb_branches ) {
				contained = meshv.region(branch).is_in(cv);
				if (!contained) branch++;
...
problem3d1d::assembly_mat(void)
{
	...
	// Local matrices
	size_type shift = 0;
    std::ofstream outcoeff("coeff_Mvv.txt");
	for(size_type i=0; i<nb_branches; ++i){
...
//cout << " ----------kvi.." <<kvi << endl;
		...
		for(size_type j=0; j<mf_coefvi[i].nb_dof(); ++j){
            ...
            max= std::max(max, ci[j]);
		}
		
		outcoeff << " max coefficients in branch i = " << i << " is == " << max <<"\n";
    
        vtk_export exp_coeff(descr.OUTPUT+"coeff_"+std::to_string(i)+".vtk");
        exp_coeff.exporting(mf_coefvi[i]);
        exp_coeff.write_mesh();
        exp_coeff.write_point_data(mf_coefvi[i], 
        ci, "coeffi");
...
/*
            cout << "computing the schur complement...." << endl;
           
    size_type tot_dof_Uvi = mf_Uvi[0].nb_dof();
    for (size_type jj = 1; jj < mf_Uvi.size(); jj++ )
        tot_dof_Uvi = tot_dof_Uvi + mf_Uvi[jj].nb_dof();
    
    
    gmm::dense_matrix < scalar_type> SS (tot_dof_Uvi, tot_dof_Uvi);
    gmm::dense_matrix < scalar_type> AA (tot_dof_Uvi, tot_dof_Uvi);
    gmm::dense_matrix < scalar_type> BB (tot_dof_Uvi, dof.Pv());
    gmm::dense_matrix < scalar_type> CC (dof.Pv(), tot_dof_Uvi);
    gmm::dense_matrix < scalar_type> DD (dof.Pv(), dof.Pv());
    
   
    gmm::copy (gmm::sub_matrix(AM,
			gmm::sub_interval(dof.Ut()+dof.Pt(), tot_dof_Uvi),
			gmm::sub_interval(dof.Ut()+dof.Pt() , tot_dof_Uvi)),AA);
    gmm::copy (gmm::sub_matrix(AM,
			gmm::sub_interval(dof.Ut()+dof.Pt(), tot_dof_Uvi),
			gmm::sub_interval(dof.Ut()+dof.Pt() + tot_dof_Uvi , dof.Pv())),BB);
    gmm::copy (gmm::sub_matrix(AM,
			gmm::sub_interval(dof.Ut()+dof.Pt()+ tot_dof_Uvi, dof.Pv()),
			gmm::sub_interval(dof.Ut()+dof.Pt()  , tot_dof_Uvi)),CC);
    gmm::copy (gmm::sub_matrix(AM,
			gmm::sub_interval(dof.Ut()+dof.Pt()+ tot_dof_Uvi, dof.Pv()),
			gmm::sub_interval(dof.Ut()+dof.Pt()+tot_dof_Uvi, dof.Pv())),DD);
    ...
    // Coupled case: DD != 0
    /*gmm::dense_matrix < scalar_type> invDD(mat_nrows(DD), mat_nrows(DD)); gmm::clear(invDD);
    gmm::copy(DD, invDD);
    gmm::lu_inverse(invDD);
    
    gmm::dense_matrix < scalar_type> SS_temp;
    gmm::dense_matrix < scalar_type> SS_temp_;
    gmm::mult(BB, invDD, SS_temp); // SS_temp= B*D^-1
    gmm::mult(SS_temp, CC, SS_temp_); // SS_temp_ = B*D^-1*C
    gmm::copy(AA, SS); //SS_temp=AA
    gmm::add(gmm::scaled(SS_temp_, -1.0), SS ); // Schur -> SS
    */
 
 /*   
    std::vector<scalar_type> eigval(mat_nrows(SS));
    scalar_type doubletol;
    gmm::implicit_qr_algorithm(SS, eigval,doubletol = 1E-10);

    std::ofstream outeig("eig_S.txt");
		outeig << gmm::col_vector(eigval);
		outeig.close();
*/        
...
// ----------------------
    ...
    scalar_type max_p =Pv[0];
    scalar_type min_p = Pv[0];
